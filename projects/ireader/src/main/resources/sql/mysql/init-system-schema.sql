...
drop table if exists `sys_user_organization_position`;;
...
create table `sys_position`(
  ...
  index `idx_sys_position_nam` (`name`),
  index `idx_sys_position_parent_id` (`parent_id`),
  index `idx_sys_position_parent_ids_weight` (`parent_ids`, `weight`)
) charset=utf8 ENGINE=InnoDB;;
alter table `sys_position` auto_increment=1000;;


create table `sys_user_organization_position`(
  `id`         bigint not null auto_increment,
  `user_id`   bigint,
  `organization_id` bigint,
  `position_id` bigint,
  constraint `pk_sys_user_organization_position` primary key(`id`),
  constraint `unique_sys_user_organization_position` unique(`user_id`, `organization_id`, `position_id`)
) charset=utf8 ENGINE=InnoDB;;
...
create table `sys_auth`(
  ...
  index `idx_sys_auth_position` (`position_id`),
